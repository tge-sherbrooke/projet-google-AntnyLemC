...
int main(int argc, char **argv) {
	...
	rc = aws_iot_mqtt_subscribe(&client, "iot/CPUTemp", 11, QOS0, iot_subscribe_callback_handler, NULL);
	if(SUCCESS != rc) {
	...
	while((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)
		...
		char temp[10];
		int n;
		char timeCustom[1000];
		time_t t = time(NULL);
    struct tm * p = localtime(&t);

    strftime(timeCustom, 1000, "%B %d %Y %X", p);
		...
		IOT_INFO("-->sleep");
		sleep(10);
		sprintf(cPayload, "{\"timestamp\": \"%s\" ,\n \"temp\": \"%s\"\n}", timeCustom,temp);
		paramsQOS0.payloadLen = strlen(cPayload);
		rc = aws_iot_mqtt_publish(&client, "iot/CPUTemp", 11, &paramsQOS0);
		if(publishCount > 0) {
			publishCount--;
		}
